...
def _downgrade_marker_to_p7c_1(config_path: Path, *, keep_screening_directories: bool = False) -> bytes:
    ...
    marker["layout_contract_version"] = "p7c-1"
    ...
    if not keep_screening_directories:
        for relative in ("screening_criteria/by_id", "screening_decisions/by_id"):
...
def test_workspace_marker_schema_and_serialization_are_deterministic(tmp_path: Path) -> None:
    ...
    assert marker["layout_contract_version"] == "p7d-1"
...
def test_old_layout_dry_run_plans_upgrade_without_mutation(tmp_path: Path) -> None:
    ...
    marker_before = _downgrade_marker_to_p7c_1(config_path)
    knowledge_root = config_path.parent / "knowledge"
    tree_before = {
    ...
    assert {tuple(item.values()) for item in result.managed_actions} >= {
        ("organization/screening_criteria/by_id", "create_directory"),
        ("organization/screening_decisions/by_id", "create_directory"),
        (".research-kb/workspace.json", "upgrade_identity_marker"),
    }
    assert (knowledge_root / "organization" / "directions" / "by_id").is_dir()
    assert not (knowledge_root / "organization" / "screening_criteria").exists()
...
def test_old_layout_apply_upgrades_only_directory_and_marker_then_converges(tmp_path: Path) -> None:
    config_path = _write_workspace(tmp_path / "workspace")
    assert WorkspaceBootstrapService(config_path).run().result == "initialized"
    _downgrade_marker_to_p7c_1(config_path)
    knowledge_root = config_path.parent / "knowledge"
    ...
    assert ("organization/screening_criteria/by_id", "create_directory") in actions
    assert ("organization/screening_decisions/by_id", "create_directory") in actions
...
def test_old_layout_upgrade_resumes_from_safe_empty_organization_directories(tmp_path: Path) -> None:
    config_path = _write_workspace(tmp_path / "workspace")
    assert WorkspaceBootstrapService(config_path).run().result == "initialized"
    _downgrade_marker_to_p7c_1(config_path, keep_screening_directories=True)
...
def test_old_layout_upgrade_rejects_nonempty_organization_directory(tmp_path: Path) -> None:
    config_path = _write_workspace(tmp_path / "workspace")
    assert WorkspaceBootstrapService(config_path).run().result == "initialized"
    marker_before = _downgrade_marker_to_p7c_1(config_path, keep_screening_directories=True)
