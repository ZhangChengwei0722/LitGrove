...
def _downgrade_marker_to_p4b_1(config_path: Path, *, keep_review_bundle_directories: bool = False) -> bytes:
    ...
    marker["layout_contract_version"] = "p4b-1"
    marker_bytes = serialize_json(marker)
    atomic_write_bytes(marker_path, marker_bytes, "test-downgrade-marker")
    review_bundle_root = knowledge_root / "review_bundles"
    if not keep_review_bundle_directories:
        (review_bundle_root / "by_paper").rmdir()
        review_bundle_root.rmdir()
    return marker_bytes
...
def test_workspace_marker_schema_and_serialization_are_deterministic(tmp_path: Path) -> None:
    ...
    assert marker["layout_contract_version"] == "p4c-1"
...
def test_old_layout_dry_run_plans_upgrade_without_mutation(tmp_path: Path) -> None:
    ...
    marker_before = _downgrade_marker_to_p4b_1(config_path)
    knowledge_root = config_path.parent / "knowledge"
    tree_before = {
    ...
    assert {tuple(item.values()) for item in result.managed_actions} >= {
        ("review_bundles", "create_directory"),
        ("review_bundles/by_paper", "create_directory"),
        (".research-kb/workspace.json", "upgrade_identity_marker"),
    }
    assert not (knowledge_root / "review_bundles").exists()
...
def test_old_layout_apply_upgrades_only_directory_and_marker_then_converges(tmp_path: Path) -> None:
    config_path = _write_workspace(tmp_path / "workspace")
    assert WorkspaceBootstrapService(config_path).run().result == "initialized"
    _downgrade_marker_to_p4b_1(config_path)
    knowledge_root = config_path.parent / "knowledge"
    ...
    assert ("review_bundles", "create_directory") in actions
    assert ("review_bundles/by_paper", "create_directory") in actions
    ...
    assert (knowledge_root / "primary_bundles" / "by_paper").is_dir()
    assert (knowledge_root / "review_bundles" / "by_paper").is_dir()
    assert read_json_document(
        knowledge_root / ".research-kb" / "workspace.json",
        record_kind="workspace-marker",
    )["layout_contract_version"] == "p4c-1"
    assert WorkspaceBootstrapService(config_path).run().result == "no_change"


def test_old_layout_upgrade_resumes_from_safe_empty_review_bundle_directories(tmp_path: Path) -> None:
    config_path = _write_workspace(tmp_path / "workspace")
    assert WorkspaceBootstrapService(config_path).run().result == "initialized"
    _downgrade_marker_to_p4b_1(config_path, keep_review_bundle_directories=True)
...
def test_old_layout_upgrade_rejects_nonempty_review_bundle_directory(tmp_path: Path) -> None:
    config_path = _write_workspace(tmp_path / "workspace")
    assert WorkspaceBootstrapService(config_path).run().result == "initialized"
    marker_before = _downgrade_marker_to_p4b_1(config_path, keep_review_bundle_directories=True)
    review_bundle_directory = config_path.parent / "knowledge" / "review_bundles"
    (review_bundle_directory / "unexpected.json").write_text("{}\n", encoding="utf-8", newline="\n")
